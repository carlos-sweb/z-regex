...
/// Cap on the generated bytecode. Counted repeats are unrolled (one copy of
/// the atom per required repetition), so nested counts multiply; past this
/// the pattern is `error.PatternTooLarge` instead of building gigabytes of
/// bytecode and overflowing the i32 jump offsets (a panic, found by the F1c
/// fuzz run). Counted loops that remove the unrolling are F5's (D10).
