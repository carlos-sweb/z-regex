...
| D12 | `find` avanza de a 1 byte y puede arrancar a mitad de una secuencia UTF-8 | `matcher.zig:162-168` | `lastIndex` avanza por code unit (o code point con `u`) | Subject (F3). **Inferido del código, no verificado ejecutándolo.** |
...
| D14 | El matcher recursivo acota la **profundidad** de recursión (1000), no los **bytes** de pila nativa: su corrección depende de la pila que le dé el llamador | test262 (F0b): `S15.10.2.8_A3_T17/T32/T33`, `S15.10.6.2_A12`, `S15.10.6.2_A1_T6` crashean con 1 MiB y pasan con 8 MiB | Una regex no debería crashear según la pila del host | T2: pila explícita en heap con límite de bytes (F6a) |
...
- **Regla de decisión:** si T2 ponderado ≥ 15 %, F6a va antes de F5; si no, después. **D14 refuerza la prioridad de F6a** con independencia del histograma: mientras el matcher sea recursivo, cualquier consumidor con poca pila (FFI, hilos) puede crashear en patrones anidados que hoy pasan con 8 MiB. F6b siempre va después de F6a. El umbral es una convención de este plan, no un dato.
...
| **F0d — Baseline y red de seguridad** | Benchmarks del backtracker actual con los casos de §7 (hoy no hay ninguno en el repo); tests de regresión de lo que hoy pasa; fuzz del parser con `std.testing.allocator` y fallos de allocación inyectados. **Arreglar `zig build test-conformance`**: el binario pasa (exit 0, 168/168), pero el runner de Zig 0.16 trata la salida a stderr como diagnóstico y marca el paso como fallido; además, el test **no tiene ninguna aserción**, así que no puede fallar aunque fallen todos los casos. **`zig fmt`** sobre `src/unicode/tables.zig`, `src/bytecode/format.zig`, `src/bytecode/opcodes.zig` y `src/utils/pool.zig`, antes de usarlo como gate. **Dato para F3/F4 (medido en F0b, no perfilado):** a través de la API C, `find` con match en la posición 0 cuesta ~62 µs frente a ~7,6 µs sin match (50 B, ReleaseSafe). La causa apunta a `src/c_api.zig` (`DebugAllocator`, copia de la entrada, 16 capturas por match), no al matcher. **Consumo de pila nativa (hallado en F0b; corrige una nota anterior que lo llamaba SIGSEGV del motor):** `/<body.*>((.*\n?)*?)<\/body>/i` (test262 `S15.10.2.8_A3_T17/T32/T33`), `/(.|\r|\n)*/` y `/(z)((a+)?(b+)?(c))*/` (`S15.10.6.2_A12`, `S15.10.6.2_A1_T6`) hacen segfault con una pila nativa de 1 MiB y pasan con 8 MiB. El matcher recursivo limita la *profundidad* (1000), no los *bytes* de pila, así que depende de la pila que le dé el host. Misma familia que `ce885bf` (grupo anulable bajo un cuantificador). F6a (pila explícita en heap con límite de bytes) lo resuelve. El harness usa 8 MiB (`ZREGEX_NATIVE_STACK_MB`). | Números base registrados; tests de regresión en CI; `test-conformance` en verde y capaz de fallar; `zig fmt --check src/` limpio. |
| **F1 — Semántica compartida correcta** | D1, D2, D3, D4, D5, D8 (posesivos a opt-in) y D9 (slots dinámicos, `\10`+ con reglas Annex B); gramática `u` estricta completa; early errors; modificadores. Se activan los tests de F0a marcados `skip`. | test262 `syntax` y `annexB` en verde (salvo lookbehind); tests post-F1 de F0a en verde. **Objetivo numérico sobre la baseline de F0b (2733/3017, 90,6 %): ≥ 2840 pass (≥ 94,1 %) y 0 regresiones.** `scripts/test262/categorize.mjs` asigna a F1 115 no-pass (Annex B 20, gramática `u`/early errors 45, nombres de grupo 18, D13 10, D9 8, D3 6, D4 6, `\xFF` 2); el objetivo admite que queden como mucho 8 de ellas, cada una documentada con su motivo. |
| **F2 — IR + Analysis** | HIR, CharSet dinámico (elimina `MAX_CLASS_RANGES` y el límite de 4 propiedades por clase); `analyze()` migra al HIR; el backtracker actual consume el HIR (una sola ruta, todavía sin Tiers); módulos de build separados por Tier. | Tests de clasificación sin cambios salvo los documentados; sin regresiones en test262. |
...
| **F6a — Tier 2 sin lookbehind** | Backtracker con pila explícita en heap (sin recursión nativa); trail; backrefs numéricas, con nombre y con `i`; lookahead (atomicidad, capturas); asserts cuantificados Annex B; `LookLinear` forward con memo; `ExecLimits` públicos (D11). Los patrones con lookbehind siguen en el backtracker actual. | test262 de backrefs, `named-groups` y lookahead en verde; ningún patrón T2 sin lookbehind pasa por `recursive_matcher.zig`; `(a+)+b` y similares cumplen la cota de §7; **el diferencial entre Tiers pasa a bloquear merges para patrones sin lookbehind**. |
...
| **F7 — Endurecimiento** | Enforcement final de dependencias en el build; suite de benchmarks; retirar `recursive_matcher.zig`; telemetría opcional de Tiers para el host. | Conformidad test262 medida y publicada en `docs/KNOWN_LIMITATIONS.md`. |
...
| Tarea | Cuándo |
|---|---|
| Convertir los bugs históricos de Phase 6 (`KNOWN_LIMITATIONS.md`) en tests permanentes, incluido el consumo de pila de `/<body.*>((.*\n?)*?)<\/body>/i` hallado en F0b (un test que lo ejecute con una pila de 1 MiB, marcado `skip` hasta F6a) | Junto con F0d |
| Límite de anidamiento del parser para grupos, clases y cuantificadores (hoy solo existe para alternancia, `parser.zig:99`) | Junto con F1 |
