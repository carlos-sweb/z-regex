...
**Tabla de clasificación de ejemplo** (es la base de los tests de F0a):
...
|---|---|---|
| **F0a — Prototipo de `analyze()` (1–2 semanas, mergeable) — hecho** | `src/analysis/classify.zig`: tabla feature→tier en código, sobre el AST actual (`NodeType` + `char_value` + flags). `analyze()` pública. Detector léxico de D1 (sin corregirlo). Los patrones afectados por D1–D3 devuelven `min_tier = null` y `reasons = [known_deviation(.D1)]` o `[parse_error]`. Sin cambios en la semántica del compilador ni backends nuevos. | Las filas de §5.2 están implementadas como tests y pasan, incluidas las de `{,5}` como `unclassifiable`. `Analysis{features, min_tier: ?Tier, reasons}` queda congelado como contrato, **que no cubre D1–D3 hasta F1**. Esas filas tienen su test post-F1 escrito y marcado como `skip`. |
...
| **F0c — Medición de 70/20/10** | `analyze()` sobre un corpus: test262 (aparte), regex extraídas de paquetes npm con un script offline en `scripts/`, y un corpus público de regex de investigación si existe y su licencia lo permite (**disponibilidad y licencia por verificar**). | Histograma publicado (único y ponderado); orden de F5/F6a decidido con la regla de §5.6. |
| **F0d — Baseline y red de seguridad** | Benchmarks del backtracker actual con los casos de §7 (hoy no hay ninguno en el repo); tests de regresión de lo que hoy pasa; fuzz del parser con `std.testing.allocator` y fallos de allocación inyectados. | Números base registrados; tests de regresión en CI. |
...
| Tarea | Cuándo |
|---|---|
| Convertir los bugs históricos de Phase 6 (`KNOWN_LIMITATIONS.md`) en tests permanentes | Junto con F0d |
