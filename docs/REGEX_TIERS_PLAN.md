...
|---|---|---|---|---|
| D1 | `{,5}` se acepta como `{0,5}` | `lexer.zig` `parseRepeat` | Annex B: `{` es literal si no forma `{n}`/`{n,}`/`{n,m}`; con `u`/`v` es SyntaxError | Parser (F1) ✅ **Cerrada en F1b.** |
| D2 | `a{` y `}` sueltos dan `error.InvalidRepeat` | `parseRepeat` | Annex B: `ExtendedPatternCharacter` literal | Parser (F1) ✅ **Cerrada en F1b.** |
| D3 | `[]` se rechaza (`error.EmptyCharClass`) | `parser.zig:863` | `/[]/` es válido y nunca matchea | Parser + CharSet vacío (F1) ✅ **Cerrada en F1b.** |
| D4 | `\s` solo ASCII `[\t-\r ]` | `parser.zig:38` | WhiteSpace + LineTerminator (U+00A0, U+FEFF, Zs, U+2028/9) incluso sin `u` | Mini-tabla fija en T0 (F1) ✅ **Cerrada en F1a.** |
| D5 | `^`/`$` con `m` y `.` solo consideran `\n` | `recursive_matcher.zig:523-541`, `:616-625` | LineTerminator = `\n`, `\r`, U+2028, U+2029 | T0 (F1) ✅ **Cerrada en F1a.** |
| D6 | Sin `u` se decodifican code points (`.` consume 😀 entero) | doc de `CompileOptions.unicode` (`compiler.zig:61-73`), `matchAnyChar` | Sin `u` se opera sobre code units UTF-16; `/^.$/.test("😀")` es `false` | Subject (F3). ✅ **Cerrada en F3d:** sin `u`/`v` un caracter es una code unit UTF-16; `/^.$/.test("😀")` da `false` en los dos encodings. |
...
| D8 | Posesivos `*+ ++ ?+` activos por defecto | `lexer.zig:49-51` | `/a*+/` es SyntaxError | Extensión opt-in (F1) ✅ **Cerrada en F1b.** |
| D9 | Máximo 16 capturas, backrefs `\1`–`\9`, `group_index: u8` | `recursive_matcher.zig:22`, `lexer.zig:34`, `ast.zig` | Sin límite práctico; `\10`+ válido si hay ≥ 10 grupos (si no, Annex B octal/identity) | Parser y slots dinámicos (F1) ✅ **Cerrada en F1c.** |
...
| D12 | `find` avanza de a 1 byte y puede arrancar a mitad de una secuencia UTF-8 | `matcher.zig:162-168` | `lastIndex` avanza por code unit (o code point con `u`) | Subject (F3). **Inferido del código, no verificado ejecutándolo.** **Parcialmente corregida en F1a:** `find`, `findAll` y `zregex_search_n` avanzan por secuencia UTF-8/WTF-8 completa (`RecursiveMatcher.nextSearchStart`), porque 2 de los 5 tests de D13 fallaban solo por esto; `lastIndex` por code unit UTF-16 sigue en F3.  ✅ **Cerrada en F3d:** sin `u`/`v` la búsqueda, `findAll` y `advanceIndex` avanzan por code unit (en WTF-8 pasan por `b+2`); con `u`/`v`, por code point. |
| D13 | Con `u`, un par surrogate escrito con escapes (`\ud834\udf06`) no se combina en un solo code point: cada `\uXXXX` queda como un surrogate suelto | test262 `built-ins/RegExp/prototype/source/value-u.js` (F0b) | Con `u`, `\uLead\uTrail` forma un solo code point (RegExpUnicodeEscapeSequence) | Parser, gramática `u` (F1) ✅ **Cerrada en F1a.** |
| D14 | El matcher recursivo acota la **profundidad** de recursión (1000), no los **bytes** de pila nativa: su corrección depende de la pila que le dé el llamador. Medido en F0d: con 1 MiB de pila, los adversariales `(a+)+b`/`(a\|aa)*c` sobre 41 bytes hacen segfault; con 8 MiB devuelven `StepLimitExceeded` en 44–48 ms (bench, ReleaseFast). **La pila del llamador es el límite entre responder y crashear.** **Medido de nuevo tras D9 (F1c):** el `MatchResult` sin capturas achicó cada frame. En ReleaseSafe, el patrón `<body…>` necesita ~103 KiB, `(a+)+b` ~159 KiB y `(a\|aa)*c` ~503 KiB, así que los tres responden con 1 MiB (el test de regresión de 1 MiB está activo). En Debug necesitan ~615 KiB, ~983 KiB y ~3,1 MiB. Los adversariales tardan 28–35 ms (antes 44–48). D14 sigue abierta hasta F6a: el límite cuenta profundidad, no bytes. | test262 (F0b): `S15.10.2.8_A3_T17/T32/T33`, `S15.10.6.2_A12`, `S15.10.6.2_A1_T6` crashean con 1 MiB y pasan con 8 MiB | Una regex no debería crashear según la pila del host | T2: pila explícita en heap con límite de bytes (F6a) |
| D15 | El contador de recursión (`max_recursion_depth = 1000`) **protege contra la profundidad, no contra los bytes de pila**, y está calibrado para una pila que ningún llamador tiene. En la cadena `matchFrom` → `matchBackRef` → `matchFrom` cada nivel cuesta **≈ 25,3 KiB en ReleaseSafe y ≈ 75,5 KiB en Debug** (medido en F0d con la pila mínima para `()\1{N}`, N = 100/200/400: crecimiento exactamente lineal), así que llegar al límite de 1000 exige **≈ 25 MiB (≈ 75 MiB en Debug)**. Con 8 MiB el crash llega antes que el límite, desde ~320 repeticiones (~105 en Debug). El contador sí cubre la ruta: con 64 MiB, `()\1{1000}` devuelve `RecursionLimitExceeded` con profundidad máxima 1000 en ReleaseSafe (Debug necesita > 64 MiB). A diferencia de D14 (el resultado depende de la pila del llamador), aquí **ninguna pila por defecto** (8 MiB en el hilo principal, menos en hilos) alcanza el límite: la protección nominal no protege nada. **Medido de nuevo tras D9 (F1c):** cada nivel cuesta **≈ 1,8 KiB en ReleaseSafe y ≈ 11,2 KiB en Debug**. Con 8 MiB, en ReleaseSafe `()\1{1000}` ya alcanza el límite y devuelve `RecursionLimitExceeded` en lugar de crashear; en Debug (~11 MiB para 1000 niveles) sigue crasheando. Ninguno de los dos da la respuesta del spec (match vacío), así que el test `skip` sigue hasta F6a. | Fuzz del parser (F0d), reducido desde `\2{9007199254740991}\[*`; test `skip` en `tests/regression_tests.zig` | Un límite de recursión que nunca se alcanza antes del crash no es un límite | T2: pila explícita en heap con límite de bytes (F6a) |
| D16 | El contador de grupos capturantes del parser es `u8` (`group_counter += 1`, `parser.zig:529`): **256 grupos capturantes secuenciales** (`(a)(a)…`) hacen panic por desbordamiento de entero en ReleaseSafe/Debug, y en ReleaseFast **desbordan en silencio a 0**, de modo que el grupo 256 escribe la captura 0 (el match completo). No lo frena el límite de anidamiento (los grupos no están anidados), y el fuzz no llega (patrones de ≤ 16 tokens). Es un bug de seguridad del código actual, medido en el Paso 0 de F1 | Paso 0 de F1: `(a)`×255 compila; ×256 y ×300, panic (ReleaseSafe) o compila con el índice desbordado (ReleaseFast) | El índice de captura no puede desbordar; un patrón válido con muchos grupos no puede crashear ni corromper el grupo 0 | **Corregida en F1c** (commit de D9, junto con D9(b)): índices u16 y `error.TooManyCaptures` por encima de 65 535; tests con 255/256/257/300 grupos que comprueban los índices del último grupo; el estrés del fuzz tiene un generador de 1–300 grupos ✅ **Cerrada en F1c.** |
...
**Tabla de clasificación de ejemplo** (es la base de los tests de F0a). En F0a también son `unclassifiable` los patrones con posesivos (D8) o con un `{n}` mayor que 65536 (D10), por el mismo criterio que D1; ver `src/analysis/classify.zig`. **Cambio de contrato en F1b (deliberado):** `Deviation` pierde `d1_quantifier_min_omitted` y `d8_possessive_quantifier`; solo queda `d10_quantifier_min_clamped` (F5). `[0-9]{,5}` sin flags es T0 y con `u` es `parse_error` (el test `skip` de F0a está activo y pasa). `a*+` es `parse_error`, porque `analyze` sigue el spec y los posesivos son un opt-in solo de `compile`. `a{` y `[]` son T0.
...
| **F1 — Semántica compartida correcta** ✅ **Cerrada (2846/3017)** | D1, D2, D3, D4, D5, D8 (posesivos a opt-in), D9 (slots dinámicos, `\10`+ con reglas Annex B), D13 y D16; gramática `u` estricta completa; early errors. **Los modificadores salen de F1** y son una fase propia, con su plan después de F1. Sub-fases, cada una con reporte y parada: **F1a** (léxico y tablas, sin cambio de AST: early errors y gramática `u`, `\xFF`, D13, D4, D5), **F1b** (pre-escaneo de grupos y nombres, D1/D2/D3, escapes de Annex B, D8, clasificador sin D1/D8), **F1c** (D9 en dos commits: (a) índices y operandos u16 con `TooManyCaptures`, que corrige D16; (b) slots dinámicos, `\10`+ y el límite de anidamiento; más nombres Unicode y referencias hacia adelante). Se activan los tests de F0a marcados `skip`. **Requisito de F1c(1), medido en el Paso 0 de F1:** subir el tamaño de los arrays fijos no sirve. `parseAtom` tiene como local un `GroupNameEntry` con `branch_path: [MAX_ALTERNATION_DEPTH]BranchStep`, y con el límite en 257 el parser pasa de ≈ 1,6 KiB a ≈ 4,9 KiB por nivel en ReleaseSafe (de ≈ 15,6 a ≈ 64,6 KiB en Debug), de modo que 256 niveles ya no caben en 1 MiB. `branch_path` y `branch_stack` pasan a memoria dinámica acotada por `nesting_depth` (o el parser a iterativo), y entonces `nesting_depth` es el límite efectivo. **D16 bloquea F1c(a):** no se llega a D9(b) sin corregirlo. **Avance: F1a hecha** (2794/3017 en la baseline: 59 entradas del objetivo, más 2 de bonus de D5; 0 misses). **F1b hecha** (2824/3017: las 26 entradas de F1b, más 4 de D9 que se adelantaron porque `\10` con 10 grupos es backreference desde F1b(4) (T8/T9); 0 misses). El pre-escaneo ya estaba en F1a(1). **Cierre de F1:** 2846/3017 en test262 (objetivo 2838; baseline de F0b 2733), 0 regresiones en ninguna sub-fase y 0 misses. F1a sumó 59 entradas + 2 de bonus de D5, F1b 26 + 4 de D9 adelantadas, y F1c 22 (18 de nombres, T16 y T15, este último como bonus: tras D9 cabe en 8 MiB). `categorize.mjs` ya no atribuye ninguna entrada a F1. Gate completo en verde: `zig build test` en Debug y ReleaseSafe (solo los skips permitidos), estrés del fuzz y corrida larga (1,5 M patrones, 0 crashes), conformance, `fmt`, bench (≈ ×3 más rápido tras D9, ningún caso peor) y `differential-v8` (0 crashes, 0 rechazos, 642 divergencias conocidas: 248 de D6 (F3) y 394 de semántica de iteración de cuantificadores (F4b)). Versión 0.2.0. **F1c, punto de partida:** 22 entradas residuales, que son 18 de nombres de grupo, T15 (2, pendiente de F6a) y T16 (2). Sin T15, el máximo alcanzable es 2844; con el objetivo en 2838, el margen es de 6 entradas absolutas (2 vienen del bonus de D5). **D9 no está hecho porque T8/T9 pasen:** falta la parte mecánica (índices u16, slots dinámicos, `MAX_NESTING_DEPTH` como límite efectivo, `branch_path` dinámico). Antes de cerrar F1c se verifica: (1) los tests de regresión de Phase 6 en verde; (2) tests con 17, 200 y 1000 grupos con capturas exactas; (3) el bench no empeora más de un 20 % en ningún caso; (4) D14 y D15 se vuelven a medir, porque el frame del matcher cambia. **Medición posterior a D9 del frame del matcher:** F1a midió ~49 KiB por grupo capturante anidado (T15). El número nuevo va a `KNOWN_LIMITATIONS.md` y a la fila de F6a como lo que F6a tiene que batir con su pila explícita. Además cerró dos bugs previos a F1 que encontraron sus tests: una doble liberación en `parseCharClass` cuando falla una asignación (`[a-]`), y `[\D]`/`[\W]`/`[\S]` dentro de una clase, que no cubrían nada por encima de U+00FF. `MAX_CLASS_RANGES` pasa de 8 a 30, con fusión de rangos en el codegen. **T15 (200 grupos *capturantes* anidados) queda pendiente de F6a:** parser, codegen y `deinit` caben en 8 MiB, pero el matcher gasta ≈ 49 KiB por grupo capturante anidado (dos frames de `matchFrom`, D15), así que 200 niveles necesitan ≈ 9,8 MiB. T16 (`(?:`, sin frame por nivel en el matcher) sí es alcanzable. | test262 `syntax` y `annexB` en verde (salvo lookbehind); tests post-F1 de F0a en verde. **Objetivo numérico sobre la baseline de F0b (2733/3017, 90,6 %): ≥ 2838 pass (≥ 94,1 %) y 0 regresiones.** `scripts/test262/categorize.mjs` asigna a F1 111 no-pass alcanzables: Annex B 20, gramática `u`/early errors 41, nombres de grupo 18, D13 10, D9 8, D3 6, D4 6, `\xFF` 2. Las 4 `S7.8.5_A1.5/A2.5` se reatribuyen al lexer de JS (host): `\` + salto de línea es inválido en un *literal*, pero válido como patrón, y zregex coincide con V8. Sin T15, el máximo es 2842 y el objetivo deja **4 misses acumulados en toda F1**, cada uno documentado al gastarlo; T15 se documenta en `KNOWN_LIMITATIONS.md` como pendiente de F6a. D5 desbloquea además 2 entradas atribuidas antes a F3 (`dotall/without-dotall-unicode`), que son un bonus fuera del objetivo. La baseline se actualiza por sub-fase con `--update-baseline-improvements`, que solo registra mejoras y se niega a escribir si hay una regresión. |
| **F2 — IR + Analysis** ✅ **Cerrada** | HIR, CharSet dinámico (elimina `MAX_CLASS_RANGES` y el límite de 4 propiedades por clase); `analyze()` migra al HIR; el backtracker actual consume el HIR (una sola ruta, todavía sin Tiers); módulos de build separados por Tier. **Objetivo explícito (de F0d): hacer el parser iterativo, o reducir la pila por nivel de anidamiento a ≤ 2 KB en Debug** (hoy ≈ 15,5 KB; ≈ 1,2 KB en ReleaseSafe). Cualquiera de las dos; al cumplirse, `MAX_NESTING_DEPTH` funciona igual en Debug y Release y se activa el test de Debug. **Medición previa, reportada antes de decidir el enfoque:** `@sizeOf` del estado del parser y de `Token` en la firma de las funciones recursivas. Si el frame del parser es > 1 KB, el problema es estructural y lo resuelve el parser iterativo; si es < 500 B, algo más consume pila (cadenas de `catch`/`errdefer`, `std.debug` en algún camino). **Requisito heredado de F1a:** decidir qué hacer con `MAX_CLASS_RANGES = 30` (el máximo que cabe hoy en el tamaño de instrucción u8): ensanchar la instrucción a u16, meter charsets dinámicos (lo que el plan original decía), o dejar 30 como límite y fallar con `PatternTooLarge` por encima. **Plan aprobado (tras F1):** sub-fases F2a (pila del parser), F2b (CharSet dinámico), F2c (HIR y lowering), F2d (`analyze()` sobre el HIR) y F2e (módulos por Tier). **Medición previa:** `@sizeOf(Parser)` = 160 B, `Token` = 48 B, `Lexer` = 48 B y `Node` = 64 B, todos por debajo de 500 B. El frame no se infla por el estado del parser, así que F2a confirma primero la causa (hipótesis: el switch de `parseAtom`) y parte sus ramas no recursivas en funciones `noinline`; no pasa a parser iterativo salvo que eso no baste. **Decisión de F2b, tabla de CharSets:** vive en memoria en `CompileResult` (`charsets`), no serializada en el bytecode. `CHAR_SET idx:u32` lleva solo el índice. `reader.zig` la lee como instrucción de 5 bytes. El optimizer no toca las tablas. Guardar el bytecode no está soportado: no es un formato estable y sin su `CompileResult` no es ejecutable; si hace falta, se hará un serializador versionado con bytecode y tablas juntos. **F2c:** el CharSet admite strings desde el inicio (vacíos hasta F5, para `\q{}`); los flags `i`/`m`/`s` se propagan con `ModifierScope{flags, body}` (scope léxico, como `(?i:…)`); un `Literal` puede tener varios caracteres. Antes de F2c y después de F2b se toma un **snapshot del bytecode** de un corpus. Política: toda diferencia se reporta; un cambio de longitud sin cambio de comportamiento se justifica en el commit y se actualiza el snapshot; un cambio de semántica es un error. `differential-v8` es la verificación semántica. **F2e es la infraestructura que habilita F4a** (VM de T0 sin `unicode/`); F0d no creó módulos por Tier. **F2a hecha (causa confirmada, sin parser iterativo).** Frames en Debug antes (`objdump`, `sub rsp`): `parseAtom` 4240 B, `parseTerm` 3216, `parseSequence` 944, `parseAlternation` 880. Suman ≈ 9,2 KiB por nivel, que coincide con la medición. Dos causas: (1) el switch de `parseAtom` y la cadena de cuantificadores de `parseTerm`, cuyas ramas no recursivas sumaban sus locales al frame de cada nivel (en Debug cada rama tiene sus propios slots); (2) `check(self: Self)` e `isQuantifierToken(self: Self)` recibían el `Parser` por valor, y en Debug cada llamada copiaba sus 160 B al frame del llamador (tres veces en `parseSequence`). Arreglo: `parseAtom` solo despacha; los grupos van a `parseGroupAtom` (frame pequeño, con el registro del nombre en `registerNamedGroup`, `noinline`), las hojas a `parseLeafAtom` (`noinline`) y los cuantificadores a `parseQuantifierSuffix` (`noinline`); `check` e `isQuantifierToken` pasan a `*const Self`. **Pila mínima de `parse` (50/100/200/256 niveles, `(` y `(?:` iguales):** Debug 469/925/1841 KiB (≈ 9,2 KiB por nivel) → 137/265/521/665 KiB con el split (≈ 2,56 KiB) → **93/177/345/441 KiB con `*const Self` (≈ 1,68 KiB por nivel)**; ReleaseSafe 53/105/209 KiB (≈ 1,04 KiB) → **25/49/97/125 KiB (≈ 0,48 KiB)**. El test "MAX_NESTING_DEPTH levels fit in a 1 MiB stack" corre ya en Debug (sin `skip`) y cubre también `(?:` y `(?<=`. Las clases `v` anidadas (`parseCharClass`, 2640 B en Debug) no suman por nivel: hoy solo se admite una operación de conjuntos, así que la recursión tiene como mucho dos niveles; se mide de nuevo cuando F5 amplíe la sintaxis `v`. **Gate después del cambio:** test262 2846, 0 regresiones; `differential-v8` idéntico a la referencia de F1 divergencia por divergencia (11.043 comparados, 642 distintos, 15 `StepLimitExceeded`, 0 crashes); `zig build test` en Debug y ReleaseSafe (solo el skip de D15); estrés del fuzz en los dos modos; bench dentro del ruido (el parser no está en la ruta de matching; compilación igual). **F2b hecha (CharSet dinámico).** `src/ir/charset.zig` (primer archivo de `ir/`, sin importar `unicode/`): rangos u32 ordenados y fusionados, con unión, intersección, diferencia, complemento y búsqueda binaria. Toda clase que no cabe en el bitmap ASCII (miembro no ASCII, `\p{…}`, operación de `v`, `[]`) se materializa en compilación y se ejecuta con `CHAR_SET`/`CHAR_SET_INV idx:u32` (0x08/0x09, 5 bytes) sobre `CompileResult.charsets`. Se eliminan los tres opcodes de tabla fija, `MAX_CLASS_RANGES`, `MAX_SET_OP_RANGES`, `MAX_CLASS_PROPERTIES`, `TooManyRanges` y `TooManyClassProperties`. Los índices se asignan en orden de primera aparición en el AST; el mapa de internado solo busca, así que la tabla es determinista. Una caché por nodo evita materializar de nuevo la clase de un repetidor desenrollado. Los bytes de la tabla cuentan en `MAX_PROGRAM_BYTES`. Un índice fuera de rango es `error.InvalidCharSet`. La C API no cambia: solo toca la tabla a través de `Regex.deinit` → `CompileResult.deinit`. Hay un test de fallos de asignación sobre la compilación de clases. **Sin cambio de semántica:** comparación vieja contra nueva de 3001 patrones (corpus de tests más 2453 clases aleatorias, flags `""`/`i`/`u`/`v`/`iu`/`iv`), 20,8 M comprobaciones y 0 diferencias. 25 patrones que antes daban `TooMany*` ahora compilan; 17 coinciden con V8 y los 8 restantes son divergencias ya documentadas (`\p` sin `u`/`v`; case folding de rangos y propiedades con `i`). **Snapshot del bytecode** (`tests/snapshots/bytecode.txt`, 679 pares flags/patrón, hash de bytecode + tabla o error de compilación), comprobado en `zig build test` y regenerado con `zig build update-bytecode-snapshot`. Es la línea base de F2c; la política y la nota sobre `[]` (242 → 5 bytes) están en su cabecera. **Gate:** test262 2846, con 0 regresiones y 0 mejoras; `differential-v8` idéntico a F1; `zig build test` en Debug y ReleaseSafe (solo el skip de D15); fuzz en los dos modos; conformance. **Bench** (mediana de 10 corridas contra `main`; resolución medida ≈ 5 %): sin empeoramientos por encima de la resolución; `[\p{L}--\p{Lu}] /v` un 7 % más rápido en matching, pero su compilación pasa de ~0,7 a ~11,5 µs. **Hallazgo corregido antes de F2c:** con `v` + `unicode`, una operación cuyo último operando es una clase entre corchetes daba `UnmatchedBracket`. Venía de F1b(2), `3c0470a`, y solo era alcanzable desde la API de Zig. El lookahead tras el `]` de una clase anidada se lee ahora con `unicode_mode` desactivado (`consumeClassClose`), y hay un test de regresión con tres casos más los controles `[a]]` y `[[a]&&[a]]]`. Gate tras el fix: test262 2846 con 0 regresiones, `differential-v8` idéntico a F1 y `zig build test` en los dos modos. El snapshot suma 20 entradas nuevas (patrones del test) y no cambia ninguna de las existentes. **F2c hecha (HIR y lowering).** `compile()` es ahora parse → lower → generate. El HIR está en `src/ir/hir.zig` (sin `unicode/`, en un arena por compilación, sin punteros al AST) y el lowering en `src/lower/lower.zig`; el codegen lee solo el HIR y el del AST desaparece. **Snapshot: 699 pares (desde `0a43ff0`), 0 diferencias.** El HIR es fiel y no canónico: `CharSet.encoding_hint` es cosmético (solo elige la codificación; lo reconocido es `set`), `Repeat.syntax_form` es semántico (`a?` limpia capturas al saltar y `a{0,1}` no) y `LitUnit.raw_byte` también lo es (un byte suelto 0x80–0xFF del patrón). Flags léxicos: solo en `ModifierScope`; en F2c hay un único scope raíz (el codegen guarda y restaura los flags; la pila llega con F5). El internado de la tabla va por (`encoding_hint`, rangos). El plegado con `i` por ruta de clase se reproduce tal cual (`[a-z]/i` acepta `A`, `[a-zé]/i` no: bug previo, documentado para F3). **Verificación:** comparación viejo contra nuevo (`git archive 0a43ff0` y los dos enlazados en un programa de scratch) sobre 40.636 patrones (corpus de tests, el de clases de F2b y 36.000 aleatorios con grupos, las 11 formas de cuantificador, backrefs, lookarounds, anclas y flags `imsuv`/posesivos): programa idéntico (hash de bytecode y tablas) o el mismo error en todos, y 633.328 `find` idénticos con capturas. Nuevo test del contrato del HIR (`tests/hir_contract_tests.zig`: el `set` de cada nodo coincide con lo que reconoce el programa compilado); detectó un bug propio del lowering (`\P{…}` suelto complementado dos veces en el HIR, invisible en el bytecode) que quedó corregido antes del commit. La comprobación de coincidencia `analyze`/`compile` de `fuzz_common.checkPattern` pasa sobre el pipeline nuevo en el corpus y en el estrés (Debug y ReleaseSafe). test262 2846 con 0 regresiones; `differential-v8` idéntico a F1. **Coste:** el estrés del fuzz destapó que el lowering materializaba cada `\p{…}` copiando su tabla y que el codegen reservaba memoria por copia desenrollada de un bitmap (`[^a]{65536}`: de 3,2 a 37,7 s de compilación en la reproducción). Corregido: las tablas de propiedades se usan en sitio (`CharSet.borrowed`, `CodepointRange`/`Range` `extern`) y los bitmaps se cachean por nodo. Resultado: +4 % en la reproducción del estrés, y el estrés en ReleaseSafe queda en 10 s (antes de F2c, 14 s). **Pila por nivel con capturas:** codegen ~0,68 KiB en Debug y ~0,12 en ReleaseSafe (antes ~0,56 y ~0,04); los `(?:…)` dejan de costar pila al codegen; el lowering nuevo gasta ~0,6 y ~0,24 KiB. **Bench** (mediana de 10 contra `0a43ff0`): matching sin cambios dentro del ruido (−5,1 %..+1,8 %); la compilación paga la pasada nueva, unos pocos µs por patrón (`hello` 0,5 → 1,1 µs, `email` 1,8 → 4,9 µs, `[\p{L}--\p{Lu}]` 11,6 → 19,6 µs). **F2d hecha (`analyze()` sobre el HIR).** `compile()` y `analyze()` comparten el front-end `lower.Frontend` (lexer, parse, nombres y lowering), así que se clasifica exactamente el HIR que se compila. El `Walker` recorre el HIR. Lo único que el HIR no sabía, la procedencia de una clase, va en `CharSetNode.analysis_origin { property, set_operation }`: solo lo lee `analyze()`. Se verificó que basta con esos dos campos: en las clases normales, "algún miembro ≥ U+0080 antes de negar" coincide con la regla del AST, y las operaciones de `v` ya fuerzan T1. **§8.2 corregido:** una desviación conocida trae features completas; `min_tier` sigue a `null`. **Cambios de features, ninguno de Tier:** se elimina `Feature.non_capturing_group` (el lowering quita `(?:…)`); `[a]` informa `literal`, no `char_class`; los miembros de una clase ya no cuentan como `literal`. El único consumidor de `Analysis` en el repo (`fuzz_common.executable`) solo lee `min_tier`. **Verificación:** los tests de clasificación existentes pasan sin cambios. Comparación de `analyze()` viejo contra nuevo (`git archive fcce61d`): 162.331 análisis, 0 diferencias de Tier y 0 de clasificabilidad; en las features solo aparecen los tres cambios documentados, más los 22 casos D10, que ahora traen features. `checkPattern` pasa en el corpus y en el estrés (Debug y ReleaseSafe); test262 2846 con 0 regresiones; el snapshot de 699 pares no cambia. **Coste:** `analyze()` ahora también baja al HIR: +57 % sobre los patrones del estrés (2,0 → 3,2 s en 60.000 análisis con `DebugAllocator`). Se mide y no se optimiza. **F2e hecha (módulos por Tier; cierra F2).** `build.zig` tiene una tabla de capas (`layers`) que es la única fuente de verdad: `ir`, `unicode` y `utils` sin dependencias; `frontend` → `ir`, `unicode`; `tier0` → `ir`, `utils` (esqueleto); `tier1` → `ir`, `unicode`, `utils`, `tier0` (esqueleto); `tier2` (el backtracker: `bytecode/`, `generator`, `optimizer`, `executor/`, `program.zig` con `CompileResult`) → `ir`, `unicode`, `utils`; `zregex` (`main.zig`, `regex.zig`, `compile.zig`, `analysis/`) → todos; `c_api` → solo `zregex`. Solo `zregex` se exporta; su API pública no cambia (se añade `zregex.tier2`) y la `.so` exporta los mismos 36 símbolos. Un binario de test por módulo; los tests que usan el pipeline completo pasan a `tests/tier2_pipeline_tests.zig`. Se ejecutan los mismos 405 tests únicos (antes 724 ejecuciones, porque los archivos compartidos con la C API corrían dos veces). **Cumplimiento, en `zig build check-layers` (paso aparte):** lint textual (`tools/check_layers.zig`, lee la tabla de `build.zig`: 43 archivos, 0 violaciones); análisis forzado por módulo (`tests/layers/ref_all.zig`, `refAllDecls` recursivo, solo compilación), que ve los imports prohibidos en código que el análisis perezoso no alcanza (verificado); y tres canarios generados por el build, sin commitear, cada uno con su mensaje exacto del compilador: `tier0` → `unicode` ("no module named 'unicode' available within module 'tier0'"), `tier1` → `tier2` ("no module named 'tier2' available within module 'tier1'") y `tier0` → `../unicode/properties.zig` ("import of file outside module path"). Cada canario tiene un control con `ir` que debe compilar. Si se concede la arista `tier0` → `unicode` en la tabla, el paso se pone en rojo (verificado). **Coste** (`zig build test` en frío, mediana de 3, Debug / ReleaseSafe): antes de F2e 16,4 / 48,5 s; con la división en módulos 20,3 / 101,1 s; con el análisis forzado dentro de `test` 23,9 / 179,4 s (+18 % / +77 %). Supera el 30 %, así que el análisis forzado vive en `check-layers`. La división en sí duplica el tiempo en ReleaseSafe (nueve binarios de test, cada uno por LLVM); se acepta. El build paraleliza (4 núcleos, sin `-j1`): en frío, ReleaseSafe gasta 117 s de reloj y 299 s de CPU (user + sys; cociente 2,6), y Debug 21 s de reloj y 42 s de CPU. Cada binario de test cuesta 24–34 s de compilación en ReleaseSafe, aunque tenga un solo test (`test-tier0`, 25 s): el coste fijo es el runner y la std por LLVM, no el tamaño del módulo. **Trigger de revisión:** si `zig build test` en ReleaseSafe (en frío) pasa de 150 s de reloj durante F3–F4, se revisa el coste de los binarios de test por módulo; antes, no. **Hallazgo del análisis forzado:** `utils/debug.zig` `formatSize` pasaba un valor de runtime a `comptimePrint`; nunca se había compilado. Arreglado (parámetro `comptime`). | Tests de clasificación sin cambios salvo los documentados; sin regresiones en test262. |
| **F3 — Subject abstracto** ✅ **Cerrada** | **D12 partido:** la mitad (avanzar por carácter) se hizo en F1a porque D13 lo requería. Queda aquí: `lastIndex` por code unit UTF-16 y el mapeo de índices entre WTF-8 y UTF-16. `Subject` WTF-8/UTF-16; semántica de code unit (sin `u`) frente a code point (con `u`) (D6); `advanceIndex`; `execAt` con `Scratch`; corregir D12. **Plan aprobado:** sub-fases F3a (módulo `subject`), F3b (bytecode independiente del encoding, solo WTF-8, sin cambio de comportamiento), F3c (matcher genérico sobre la unidad, `Scratch`, `execAt`, C API y harness con `--encoding`), F3d (D6 y D12; **absorbió F3e**: F3d(3) es el cierre de F3, el gate de F3d cierra la fase entera y su reporte es el de cierre de F3). **F3a hecha.** Módulo `subject` (`src/subject/root.zig`, sin dependencias; lo importan `tier0`, `tier1`, `tier2` y `zregex`): `Subject{wtf8, utf16}` con índices en sus unidades, `Mode{code_unit, code_point}`, `decodeAt`/`decodeBefore`, `isPosition`, `advanceIndex` (AdvanceStringIndex), `wtf8ToUtf16Index`/`utf16ToWtf8Index` y los transcodificadores para tests. **Posiciones de WTF-8:** las fronteras de secuencia y, dentro de una secuencia de 4 bytes (siempre un par lead+trail), `b+2` para el punto entre las dos mitades; ninguna otra posición interior. Un lead suelto seguido de otro lead, o de un caracter BMP, son unidades de 3 bytes sin `b+2`; un lead y un trail codificados por separado (WTF-8 mal formado) son dos surrogates sueltos que no se combinan ni con `u`; un byte inválido es una unidad con su valor. En `b+2` se decodifica la mitad trail en los dos modos (como el spec con `u` y `lastIndex` a mitad de par). Tests: todos los code points (incluidos los surrogates) en los dos encodings y los dos modos, cada caso de la tabla de posiciones, ida y vuelta de índices y 2000 strings aleatorios comparando WTF-8 con UTF-16. Los strings con surrogates de test262 son todos surrogates sueltos (`\uD800`, `\uDFFF`) o pares con match en la mitad (`coerce-unicode.js`, `builtin-infer-unicode.js`, `u-advance-after-empty.js`, `u-lastindex-adv.js`, …); ninguno trae lead+lead ni lead+BMP, que quedan cubiertos solo por los tests propios. **F3b hecha (bytecode independiente del encoding).** El matcher lee todo con `subject.decodeAt`/`decodeBefore` en modo `code_point` (la semántica previa a F3 para todos los patrones): literales, clases, conjuntos, `\p`, `.`, terminadores, `\b`, backrefs y lookbehind. Un literal por encima de U+007F es un solo `CHAR32` con su code point (antes, uno por byte); un byte crudo del patrón es el opcode nuevo `BYTE`. `Decoded.invalid` marca un byte WTF-8 mal formado, que un literal nunca iguala. **Snapshot:** 73 de 699 cambian, exactamente los patrones con literal no ASCII o `BYTE` (verificado sobre el programa de cada entrada). **Viejo contra nuevo** (F3a, 40.636 patrones, `find` y `findAt` en cada offset de 22 sujetos con astrales, surrogates y bytes mal formados): sin diferencias de compilación ni de error; 35 diferencias por un bug corregido (`^` con `m` aceptaba un LF/CR suelto tres bytes antes: `/^x/m` igualaba en `"\nabx"`, con test de regresión) y 300 de lookbehind (arranca de caracter en caracter, nunca a mitad de uno, hasta 100 caracteres en vez de 100 bytes); `findAt` en `b+2` (13.543) o en otro offset interior (1.970, que no es posición) difiere por diseño. **Gate:** test262 2846 con 0 regresiones, `differential-v8` idéntico a F1, fuzz en los dos modos, conformance, contrato del HIR. **Bench:** la primera versión perdía entre 17 y 34 % en ASCII y 53 % en lookbehind (todo pasaba por la ruta genérica de decodificación); con el camino rápido ASCII inline y el test de caracter compartido, mediana de 10 contra F3a de −1,5 % a +25,8 % (lookbehind +25,8 %, backref +15,6 %, literal +9,6 %), nada peor que la resolución. **Pendiente (decidido tras F3a):** antes de F4a (en F3e o al arrancar F4a, no en mitad de F3), un commit de infraestructura une `test-ir`, `test-unicode`, `test-utils` y `test-subject` en un solo `test-leaves` (las cuatro son hojas), para ahorrar ~75 s de CPU en ReleaseSafe y dejar margen frente al trigger de 150 s. **Lookbehind +25,8 %, medido:** sobre el sujeto del bench (solo ASCII) viejo y nuevo dan los mismos pasos del matcher (1.233 = 1.233), así que la ganancia es de coste por paso (decodificación inline, `CHAR32` directo). Sobre texto no ASCII sí hay menos pruebas (griego: 2.240 → 1.508 pasos, −33 %), porque ya no se arranca a mitad de caracter. Casos que antes salían mal en silencio: `(?<=(.))x` sobre `"éx"` capturaba el byte `A9` (medio caracter) y ahora `é`; `(?<=([^a]))x` sobre `"😀x"` capturaba `80` y ahora `😀`. **Regla para F3c (lección de F3b):** la ruta ASCII tiene que seguir siendo monomorfa. La primera versión de F3b perdía entre 17 y 53 % por pasar cada caracter por la decodificación genérica. Cuando el matcher pase a `RecursiveMatcher(comptime Unit)`, la especialización comptime es la que paga el camino rápido (una comparación `unit < 0x80` sin despacho por encoding ni por union). No es un parche: es la razón por la que `Unit` es comptime y no runtime, y un `Unit` en runtime (umbral 30–100 % del cierre 5) solo se considera si no rompe esa condición. **Para F3c:** un test explícito toma una muestra de los `findAt` que caen dentro de un caracter (los 15.513 de la comparación de F3b) y confirma que ahora devuelven `null`. **Para F3e:** una sección en `KNOWN_LIMITATIONS.md` que resuma por fase las categorías de cambios justificados del snapshot (F2b `CHAR_SET`, F3b `CHAR32`, F3c `Unit`, F3d code unit). **F3c(1): coste de compilar dos instancias del matcher.** `RecursiveMatcherFor(comptime Unit)` con `u8` y `u16`. Medido con `zig build test` en ReleaseSafe, en frío, mediana de 3, sobre dos copias del árbol: A sin instancia `u16` y B con la instancia `u16` forzada en todo binario que compile el matcher (el peor caso, que es el estado final de F3c). Reloj 124,3 → 130,6 s (**+5,1 %**); CPU (user + sys) 326,9 → 328,3 s (**+0,4 %**). Por debajo del 30 %: aceptable, `Unit` sigue en comptime. El reloj de A ya es 124 s (en F2e fue 101 y en el Paso 0 117, con la capa `subject` en medio): quedan unos 20 s hasta el trigger de 150 s, lo que refuerza unir las hojas en `test-leaves` antes de F4a. **F3c hecha (matcher genérico, `Scratch`, `execAt`, C API y harness).** (1) `RecursiveMatcherFor(comptime Unit)`: `u8` = WTF-8, `u16` = UTF-16; el camino rápido inline es el especializado (ASCII en `u8`, cualquier unidad que no sea surrogate en `u16`). (2) API: `Regex.execAt(subject, index, *Scratch, *MatchSlots, ExecLimits)`, `advanceIndex`, `groupCount`, `slotCount`; `zregex.Scratch`, `MatchSlots`, `ExecLimits` (= el `ExecOptions` de antes, presupuesto por posición de arranque hasta F6a), `ExecError` (`InvalidIndex` para un índice dentro de un caracter, `SlotsTooSmall`). `Scratch` guarda todo lo que asigna una ejecución; con uno caliente, 0 asignaciones en los casos del bench (test con allocator contador, en los dos encodings); `in_use` hace panic en builds seguros. `CompileResult.mode` (`code_point` con `u` o `v`) queda cableado desde `CompileOptions` (tests por `compile`, `compileWithOptions`, `zregex_compile` y `zregex_compile_n`); el matcher lo usa desde F3d. La fachada de bytes corre sobre `Matcher.exec`; `findAt` dentro de un caracter devuelve `null` (296.900 casos del viejo contra nuevo, todos `null`; test con muestra). (3) C API: `zregex_exec_{wtf8,utf16}` y `zregex_advance_index_{wtf8,utf16}` (40 símbolos, los 36 de antes intactos); `zregex_search_n` hace una sola búsqueda (la versión que llamaba `findAt` por posición provocó 6 timeouts en `CharacterClassEscapes`). Harness con `--encoding`: UTF-16 sin mapeo y WTF-8 con la convención `b+2`, que sustituye al parche de mitad de par; default `wtf8` hasta F3d (decisión del cierre 4 documentada en `scripts/test262/README.md`). (4) Los mismos tests en los dos encodings: `expectMatch` de `syntax_tests`, los 168 casos de `test262_data` (`tests/subject_tests.zig`), un diferencial interno WTF-8 frente a UTF-16 (`tests/differential.zig`, §7.1) y el fuzz (sujetos astrales y con surrogates, y comparación de encodings en cada patrón ejecutado). La comparación encontró que el lexer partía en bytes un surrogate escrito en WTF-8 dentro del patrón y el escape `\` de un caracter no ASCII; ahora son un caracter (`/\é/i` pasa a igualar `É`, como en V8). **Gate:** test262 **2848 en los dos encodings** (+2: `Symbol.replace/coerce-unicode.js`, el caso de `lastIndex` a mitad de par; estados idénticos en los 3996 tests entre encodings), 0 regresiones; `differential-v8` en WTF-8 idéntico a F1 y en UTF-16 idéntico a WTF-8; fuzz en los dos modos; conformance; `examples/`; `check-layers`. **Bench** (mediana de 10 contra F3b): de +10 a +38 % en la mayoría (literal +31 %, sparse +38 %, email +17 %); `[\p{L}--\p{Lu}]` −7,6 % con rango ancho (7,56–9,36 frente a 8,63–9,57); lookbehind −10,8 %, atribuido a disposición del código: medido intercalado, 35c4f79 da 0,59–0,60 y el commit siguiente, que solo cambia `c_api.zig` (fuera del bench) y el lexer para bytes ≥ 0x80 (fuera del patrón), ya da 0,54–0,55. **Decisiones del cierre de F3c:** `[\p{L}--\p{Lu}]` −7,6 % se acepta como ruido (rangos solapados). El lookbehind −10,8 % se acepta por ahora y queda documentado en `KNOWN_LIMITATIONS.md`; **si en F3e sigue en −10 % o peor, se investiga** (ensamblado del bucle interno, hints de inline); si no, se revisa en F4a con la Pike VM. **Aclaración de números:** las 15.513 de F3b son las diferencias entre F3a y F3b en `findAt` dentro de un caracter (13.543 en `b+2` y 1.970 en offsets que no son posición); las 296.900 de F3c son las llamadas a `findAt` en offsets que no son posición cuyo resultado cambió entre F3c(1) (que arrancaba ahí y casi siempre encontraba algo con `.`, `\S` o el patrón vacío) y F3c(2) (`null`). Las dos cuentan sobre 40.636 patrones × 22 sujetos × cada offset de cada sujeto, no casos únicos. **Para F3e:** `.gitattributes` (`linguist-generated`) para `tests/test262_data.zig` y excluirlo de `zig fmt` (en F3c un `zig fmt tests/*.zig` lo reformateó; se restauró antes del commit). `bench/README.md` recoge cómo comparar (10 corridas intercaladas, mediana), la regla de no compilar mientras corre un bench y, desde F3d, que un caso ASCII que empeora más de un 10 % es un bug. **`zig build test262-wtf8` (desde F3d):** el mismo test262 con sujetos WTF-8 contra `scripts/test262/baseline-wtf8.json`. Es un **paso del gate de cierre de cada fase (F3, F4b, F5, F6a), no de cada commit ni de CI**; `zig build test262` (UTF-16, `baseline.json`) sigue siendo el gate por commit. Pasar en WTF-8 y fallar en UTF-16 es un bug del motor; al revés, del harness. **Tres estados:** (1) 0 diferencias de estado entre encodings: verde; (2) diferencias documentadas con su justificación: se aceptan, se anotan en `KNOWN_LIMITATIONS.md` y se actualiza `baseline-wtf8.json`; (3) diferencias sin documentar: bloquean la fase. | Los mismos tests en ambos encodings.  **F3d hecha (D6 y D12; cierra F3).** (1) Lexer: flag `code_units` (`!(u or v)`, separado de `unicode_mode`); sin `u` un astral del patrón son dos tokens (estado en la posición, `b+2`) y un `\u{...}` astral también (trail pendiente). Matcher: decodifica según `CompileResult.mode`; búsqueda, `findAll` y `advanceIndex` avanzan por el modo. Como en V8, `[😀-🙏]` sin `u` es `SyntaxError`. Test del contrato del HIR: sin `u` y por encima de U+FFFF comprueba la mitad lead (el nodo iguala exactamente esa mitad si y solo si el conjunto la contiene). Viejo contra nuevo: 0 diferencias con `u`/`v`, 0 fuera de "patrón sin `u` con astral"; `analyze()` solo cambia en los 1.240 rangos astrales que pasan a `InvalidCharRange`. Snapshot: 4 de 699. (2) Harness en UTF-16 por defecto; `zig build test262-wtf8` con `baseline-wtf8.json` como paso de gate de fase. test262 **2856 en los dos encodings** (+8), estados idénticos en los 3996 tests. (3) **Diferencial con V8:** la clasificación de F1 de las 248 "D6" era por condición (sujeto con astral sin `u`), no por causa: 106 eran D6 puras y desaparecen; 142 tienen además un grupo cuantificado y se reclasifican a F4b (135 sin cambio y 7 que solo cambian en capturas). Aparecen 2 `StepLimitExceeded` (sin `u` un astral son dos caracteres y dos patrones exponenciales pasan el presupuesto por arranque, D11; con presupuesto ilimitado coinciden con V8; los resuelve F6a). Nueva referencia en el repo: `tests/differential/reference/diff-F3d.json` (536 distintos, todos F4b; 17 `StepLimitExceeded`; 0 D6). **Bench** (F3d(1) contra F3c): casos ASCII entre +1,0 y +5,8 %; el lookbehind vuelve a 0,59 (la caída de F3c era disposición del código); `(a+)+b` −8,8 % de *tiempo* (más rápido; mismo bytecode, 1.000.000 de pasos en los dos). **Pendientes que el plan dejaba "para F3e", resueltos en F3d(3):** la tabla de cambios justificados del snapshot por fase está en `KNOWN_LIMITATIONS.md`; `.gitattributes` marca `tests/test262_data.zig` como generado y `CONTRIBUTING.md` pide no pasarle `zig fmt`; el lookbehind ya no está en −10 % (0,59 en F3d(1)), así que la condición de investigarlo no se activa; `test262-wtf8` corrió en el gate de cierre. **Hecho en el Paso 0 de F4a:** las hojas se unen en `test-leaves` (125,0 → 98,0 s de reloj y 328,1 → 254,2 s de CPU en ReleaseSafe, en frío, mediana de 3). |
| **F4a — VM lineal sin capturas** | **Empieza en `src/tier0/` (F2e), que solo puede importar `ir`, `utils` y `subject` (F3a);** `zig build check-layers` lo comprueba. **Decidido en F2c:** T0 genera su propio `Program` desde el HIR y nunca reutiliza el bytecode del backtracker; lee solo `CharSet.set` y no lee ni `encoding_hint` ni `syntax_form`. Un opcode de literal de varios caracteres es trabajo de este `Program`, no del backtracker. Caracteres, clases, anclas, alternancia, cuantificadores sin capturas internas; prefiltros (incluida la búsqueda de literal con `std.mem.indexOfScalar`); fast path de clase única; `existsAnchoredMatch`. Los patrones con capturas siguen yendo al backtracker. | Diferencial contra el backtracker con 0 discrepancias en patrones sin capturas (corpus + fuzz); sin regresiones en test262; objetivos de §7 para T0 sin capturas; overhead dentro de los límites de §7 (si no, aplica la regla de §7).  **Paso 0 hecho:** `test-leaves` (ver la fila de F3); gate de entrada fijado: la referencia es `tests/differential/reference/diff-F3d.json` y cualquier divergencia nueva es un bug del ejecutor nuevo. **Gate añadido en F3d:** F4a cambia el ejecutor, no la semántica: `differential-v8` no puede añadir divergencias respecto de `tests/differential/reference/diff-F3d.json` (ciclo de vida de la referencia en `scripts/test262/README.md`). **Elegibilidad medida en F4a(1)** (`tier0.check` sobre los 40.636 patrones del corpus de F2c): **7.453 elegibles = 79,5 % de T0 (9.376), 18,3 % del corpus**. Son los números esperados, no un problema: la métrica es la cobertura de T0; el porcentaje sobre el corpus refleja su reparto (14.799 T1 + 11.762 T2, el 65 %), no el de regex reales (eso lo mide F0c). Rechazos dentro de T0: captura 1.797 (F4b), repeat iterado sobre cuerpo anulable 122 (F4b), byte crudo 4. Desglose por fase en `KNOWN_LIMITATIONS.md` § F4a. |
| **F4b — Capturas y semántica fina** | Capturas etiquetadas, prioridad greedy/lazy, regla de iteración vacía, `ClearSlots`. T0 aplica la repetición del spec y no lee `Repeat.syntax_form`; entre F4b y F6a, el diferencial entre Tiers puede mostrar la diferencia con el backtracker en grupos cuantificados (`(a){0,1}` frente a `(a)?`, capturas por iteración), que se atribuye a T2 (§5.5) y está documentada en `KNOWN_LIMITATIONS.md`. Si F4b alinea también el backtracker con el spec, `syntax_form` puede desaparecer aquí. | Diferencial completo sin discrepancias, o con discrepancias atribuidas a T2 y confirmadas por test262 (§5.5); sin regresiones en test262; overhead dentro de §7.  **Desde F3d** la categoría "semántica de iteración de cuantificadores" del diferencial con V8 cuenta **536** divergencias (394 de F1 más 142 que F1 había contado como D6 por condición; 7 de ellas difieren solo en capturas). |
...
| Divergencia entre Tiers | todos | Un solo parser/HIR/CharSet/Canonicalize; T1 sin ejecutor propio | Parcial (F2): un solo parser, HIR y CharSet; T0/T1 sin ejecutor todavía |
| Dependencias hacia arriba | todos | Módulo de build por capa (`tier0` sin `unicode`): una importación indebida no compila | Existe (F2e): tabla `layers` de `build.zig`; `zig build check-layers` con lint, análisis forzado y canarios |
| Asignaciones y fugas en exec | todos | Todo el estado de ejecución vive en un `Scratch` reutilizable; sin allocs por exec en el caso normal | Existe (F3c): `Scratch` reutilizable, 0 asignaciones por ejecución con uno caliente (test con allocator contador) |
...
| Condiciones de carrera | todos | `Regex` inmutable; `Scratch` con `in_use` en Debug | Existe (F3c): `Regex` inmutable; `Scratch.in_use` hace panic en builds seguros |
...
| Tarea | Cuándo |
|---|---|
| ✅ **Hecho en F2d.** `analyze()` deja de recoger features al encontrar una desviación conocida: `\2{9007199254740991}` sale `known_deviation` (D10) sin registrar la backreference, así que su Tier queda sin determinar. El fuzz no ejecuta los no clasificables por esto. **Desde F2d** el recorrido del HIR no se corta: una desviación conocida trae sus features completas (`(a)\1{70000}` registra la backreference); `min_tier` sigue a `null` hasta F5, así que el fuzz sigue sin ejecutarlos | F2 (`analyze()` migra al HIR) |
