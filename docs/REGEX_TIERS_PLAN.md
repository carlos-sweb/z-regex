...
- **Regla de decisión:** si T2 ponderado ≥ 15 %, F6a va antes de F5; si no, después. F6b siempre va después de F6a. El umbral es una convención de este plan, no un dato.
...
| **F0b — test262 real** | Harness con Node + FFI (`c_api.zig`) sobre `built-ins/RegExp`, `language/literals/regexp` y `annexB`; baseline del porcentaje por directorio. | Corre en CI; baseline registrada; es la puerta de todas las fases siguientes. |
...
| **F1 — Semántica compartida correcta** | D1, D2, D3, D4, D5, D8 (posesivos a opt-in) y D9 (slots dinámicos, `\10`+ con reglas Annex B); gramática `u` estricta completa; early errors; modificadores. Se activan los tests de F0a marcados `skip`. | test262 `syntax` y `annexB` en verde (salvo lookbehind); tests post-F1 de F0a en verde. |
